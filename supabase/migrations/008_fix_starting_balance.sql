...
-- Change default balance to 1,000 K-Coins and update trigger

-- Update the default on the users table
ALTER TABLE public.users ALTER COLUMN balance SET DEFAULT 1000;

-- Update the trigger function to use 1,000
CREATE OR REPLACE FUNCTION public.handle_new_user_v2()
RETURNS trigger AS $$
BEGIN
  INSERT INTO public.users (id, username, balance)
  VALUES (NEW.id, COALESCE(NEW.raw_user_meta_data->>'username', NEW.email), 1000)
